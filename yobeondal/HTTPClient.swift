...
class HTTPClient {
    ...
    func getMemos(_ year: Int,_ month: Int, completion: @escaping (Result<[Memo], NetworkError>) -> Void) {
        guard let url = URL.getMemosUrl(year, month) else {
            return completion(.failure(.badURL))
        }
    ...
    func createMemo(_ year: Int,_ month: Int,_ content: String,_ userId: Int, completion: @escaping (Result<String, NetworkError>) -> Void) {
        
        guard let url = URL.postMemosUrl() else {
            return completion(.failure(.badURL))
        }
        
        let memoData: [String: Any] = ["content" : content, "userId" : userId, "year" : year, "month" : month]
        guard let jsonData = try? JSONSerialization.data(withJSONObject: memoData) else { return }
